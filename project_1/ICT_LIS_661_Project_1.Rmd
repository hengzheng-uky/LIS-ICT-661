...
This is an R Markdown document (abbreviated as `.Rmd`). We've already done some exploration of R Markdown, but it's worth going back over some of the basics here. Markdown is a simple markup language (notice the pun?) that is popular in many ICT communities, including data science. A markup language is a way of using code to format text.

For example, in Markdown, putting a single asterisk on either side of some text makes it *italics*, putting two asterisks on either side of some text makes it **bold**. You can also use Markdown to format text in other ways, like adding a hyperlink to this [helpful cheatsheet on R Markdown](https://posit.co/wp-content/uploads/2022/10/rmarkdown-1.pdf). 
...
That said, it's often useful to run the code before you knit the file—you know, to make sure the code works properly before you publish it. In that case, you can hit the green "play" button in the top right of the code chunk, and the code will run and display the results in a mini-console attached to the code itself. Try it out on the code-chunk above. You'll need to run your own code to test it as you work with `.Rmd` documents in this class.
